...
template <class TCovarianceUpdate, class TGenoPheno>
CMAStrategy<TCovarianceUpdate, TGenoPheno>::~CMAStrategy() {
  if (!eostrat<TGenoPheno>::_parameters._fplot.empty()) delete _fplotstream;
}
...
bool CMAStrategy<TCovarianceUpdate, TGenoPheno>::stop() {
  ...
  if (!eostrat<TGenoPheno>::_parameters._fplot.empty()) plot();

  if (eostrat<TGenoPheno>::_niter == 0) return false;
...
int CMAStrategy<TCovarianceUpdate, TGenoPheno>::optimize(
  ...
  if (eostrat<TGenoPheno>::_parameters._with_edm) eostrat<TGenoPheno>::edm();
